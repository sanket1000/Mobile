...
{
    internal class APInvoiceSettings
    {
        #region Private
        ...
        static volatile APDistributionSettings _apDistributionSettings;
        public static APDistributionSettings APDistributionSettingsStatic
        {
            get
            {
                if (_apDistributionSettings == null)
                {
                    lock (syncRoot)
                    {
                        if (_apDistributionSettings == null)
                        {
                            _apDistributionSettings = new APDistributionSettings();
                        }
                    }
                }
        ...
        private void Init()
        {
            ...
            adoContRs = APDistributionSettingsStatic.APM_MASTER__AP_Controls;
        ...
        private void GetCustomDescriptions()
        {
            ...
            if (miscDed || invoiceCode1 || invoiceCode2)
            {
                ...
                sQry = "SELECT * FROM APM_MASTER__INVOICE where vendor='.~' and invoice='.~'";
                aStdRs = new DataTable();
                APIUtilDataAccessHelper.TimberlineStandardData.FillDataTable(aStdRs, sQry, null, true, false);
                
                sQry = "SELECT * FROM MASTER_APM_INVOICE WHERE vendor='.~' and invoice='.~'";
                aCusRs = new DataTable();
                APIUtilDataAccessHelper.TimberlineCustomData.FillDataTable(aCusRs, sQry, null, true, false);
